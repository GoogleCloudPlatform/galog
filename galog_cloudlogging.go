...
const (
	...
	// The following are labels used for MIGs.
...
// parseMIG parses the MIG string gotten from MDS and returns the labels to be
// used for cloud logging.
func (cb *CloudBackend) parseMIG(mig string) map[string]string {
	...
	switch matches[1] {
	case "zones":
		locationLabel = managedInstanceGroupZoneLabel
	case "regions":
		locationLabel = managedInstanceGroupRegionLabel
	}
	labels[managedInstanceGroupZoneLabel] = matches[3]
...
func (cb *CloudBackend) InitClient(ctx context.Context, opts *CloudOptions) error {
	...
	if opts.Instance != "" {
		...
		extraLabels := cb.parseMIG(opts.MIG)
